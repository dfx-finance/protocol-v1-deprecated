...
import { mintCADC, mintEURS, mintNZDS, mintUSDC, mintXSGD, mintXIDR, mintEUROC } from "./Utils";
...
export const scaffoldTest = async () => {
  ...
  const EurocToUsdAssimilator = await ethers.getContractFactory("EurocToUsdAssimilator");
  ...
  const eurocToUsdAssimilator = await EurocToUsdAssimilator.deploy();
  ...
  const euroc = (await ethers.getContractAt("ERC20", TOKENS.EUROC.address)) as ERC20;
...
export const scaffoldHelpers = async ({ curveFactory, erc20 }: { curveFactory: CurveFactory; erc20: ERC20 }) => {
  ...
  const mintAndApprove = async function (
    ...
    if (tokenAddress.toLowerCase() === TOKENS.EUROC.address.toLowerCase()) {
      await mintEUROC(minterAddress, amount);
    }
